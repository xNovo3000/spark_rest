...
Future sparkBoot({
  ...
  for (var ep in endpoints) {
    router.appendRoute(
      endpoint: ep,
      requestMiddlewares: List.generate(ep.requestMiddlewares.length,
          (index) => _handleMiddlewareGet(index, requestMiddlewares, ep),
          growable: true),
      responseMiddlewares: List.generate(ep.responseMiddlewares.length,
          (index) => _handleMiddlewareGet(index, responseMiddlewares, ep),
          growable: true),
    );
  }
  // register widespread request middlewares
  requestMiddlewares.forEach((_, middleware) =>
      middleware.appendOverride != null
          ? router.appendMiddleware(middleware, middleware.appendAtBegin)
          : null);
  // register widespread response middlewares
  responseMiddlewares.forEach((_, middleware) =>
      middleware.appendOverride != null
          ? router.appendMiddleware(middleware, middleware.appendAtBegin)
          : null);
  // init everything
  ...
  return HttpServer.bind(InternetAddress.anyIPv4, port)
      .then((httpServer) => httpServer.listen((httpRequest) async {
...
Middleware<T> _handleMiddlewareGet<T>(
    int index, Map<String, Middleware<T>> map, Endpoint endpoint) {
  Middleware<T> mw;
  if (T == Request) {
